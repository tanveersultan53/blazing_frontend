...
import { Loader2, ArrowLeft, FileImage, FileCode, FileText } from 'lucide-react';
import PageHeader from '@/components/PageHeader';
import { createDefaultEmail, updateDefaultEmail, getDefaultEmail, deleteDefaultEmail, distributeEcard } from '@/services/ecardService';
...
export default function EcardForm() {
  ...
  const [ecardImagePreview, setEcardImagePreview] = useState<string | null>(null);
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success(`Ecard ${isEditMode ? 'updated' : 'created'} successfully!`);
      navigate('/ecards');
    },
    onError: (error: any) => {
      console.error('Save ecard error:', error);
      toast.error(error.response?.data?.error || `Failed to ${isEditMode ? 'update' : 'create'} ecard`);
    },
  });
  ...
  const { data: ecardsData } = useQuery({
    ...
    enabled: showEditListDialog || showDeleteListDialog || showPreviewListDialog,
  });
  ...
  return (
    <PageHeader
      ...
      description={isEditMode ? 'Update ecard information' : 'Create a new ecard or email template for all users'}
      actions={[
        {
          label: 'Back to List',
    ...
    >
          ...
          <CardContent className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-4">
                <Button
                  type="button"
                  variant={formData.email_type === 'ecard' ? 'default' : 'outline'}
                  onClick={() => handleInputChange('email_type', 'ecard')}
                >
                  Ecard
                </Button>
                <Button
                  type="button"
                  variant={formData.email_type === 'email' ? 'default' : 'outline'}
                  onClick={() => handleInputChange('email_type', 'email')}
                >
                  Normal Email
                </Button>
              </div>
            ...
            {/* Row 1: Email Name & Email Subject */}
            ...
            {/* Row 2: Email Preheader & Email Category */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {/* Email Preheader */}
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => handleInputChange('email_preheader', e.target.value)}
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) => handleInputChange('email_category', Number(value))}
                >
                  <SelectTrigger id="email_category">
            ...
            {formData.email_type === 'ecard' && formData.email_category !== 1 && formData.email_category !== 0 && (
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => handleInputChange('ecard_date', e.target.value)}
                />
                <p className="text-xs text-muted-foreground">
                  The date when the ecard should be sent (e.g., 7/4/2026 for 4th of July)
                </p>
              </div>
            )}
            ...
            {formData.email_type === 'ecard' && !uploadedHtmlFile && (
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  onChange={(e) => handleInputChange('ecard_text', e.target.value)}
                  rows={6}
                />
                <p className="text-xs text-muted-foreground">
            ...
            {/* Row 3: Upload Image & Upload Email HTML */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {/* Upload Image */}
              <div className="space-y-2">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  type="file"
                  accept=".html,.htm"
                  onChange={(e) => handleHtmlFileUpload(e.target.files?.[0] || null)}
                />
                <p className="text-xs text-muted-foreground">
            ...
            <div className="flex items-center space-x-2">
              <Switch
                ...
                onCheckedChange={(checked) => handleInputChange('custom_email', checked)}
              />
          ...
          <div className="space-y-2">
            {ecardsList.map((ecard) => (
              ...
              >
                ...
                <div className="text-sm text-muted-foreground">{ecard.email_subject}</div>
              </div>
            ))}
            {ecardsList.length === 0 && (
      ...
      {/* Preview List Dialog */}
      <Dialog open={showPreviewListDialog} onOpenChange={setShowPreviewListDialog}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
      ...
      <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This action cannot be undone. This will permanently delete the ecard
              "{selectedEcard?.email_name}" from the system.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
          ...
          <div className="overflow-auto max-h-[70vh] rounded-lg border-2">
            {selectedEcard?.email_html ? (
              <iframe
                ...
                sandbox="allow-same-origin"
              />
            ) : selectedEcard?.ecard_image && typeof selectedEcard.ecard_image === 'string' ? (
              <img
